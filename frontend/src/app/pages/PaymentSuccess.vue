...
onMounted(async () => {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    const result = await ordersStore.verifyOrder(sessionId)

    if (result.success && result.order?.paymentStatus === 'paid') {
      verified.value = true
      verifying.value = false
      return
    }

    if (result.success && result.order?.paymentStatus === 'failed') {
      break
    }
...
<template>
  <main class="min-h-screen bg-gray-50 dark:bg-[#0c1210] pt-24 flex items-center justify-center px-4">
    ...
    <div v-if="verifying" class="text-center max-w-sm">
      ...
      <p class="text-gray-500 dark:text-[#7a9e8c] text-sm">Please wait while we confirm your payment.</p>
    </div>
    ...
    <div v-else class="text-center max-w-sm">
      ...
      <h2 class="text-gray-900 dark:text-[#e2efe8] text-2xl mb-3" :style="{ fontWeight: 800 }">Verification Failed</h2>
