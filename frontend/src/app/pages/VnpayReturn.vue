...
onMounted(async () => {
  ...
  orderId.value = route.query.vnp_TxnRef as string

  if (!responseCode || !orderId.value) {
    error.value = 'Invalid payment response'
    verifying.value = false
    return
  }

  // VNPay response code '00' means payment was successful
  if (responseCode !== '00') {
    error.value = responseCode === '24'
  ...
  // Payment reported as successful by VNPay — poll order status
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    const order = ordersStore.orders.find(o => o._id === orderId.value)
  ...
  // VNPay already confirmed responseCode=00, so payment is successful
...
<template>
  <main class="min-h-screen bg-gray-50 dark:bg-[#0c1210] pt-24 flex items-center justify-center px-4">
    ...
    <div v-if="verifying" class="text-center max-w-sm">
      ...
      <p class="text-gray-500 dark:text-[#7a9e8c] text-sm">Please wait while we confirm your VNPay payment.</p>
    </div>
